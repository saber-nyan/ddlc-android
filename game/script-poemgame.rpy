...
label poem(transition=True):
    ...
    if persistent.playthrough == 0 and chapter == 0:
        call screen dialog("Пришло время написать поэму!\n\nВыбирай слова, которые, по-твоему, соответствуют\nпонравившемуся тебе члену клуба. Нечто хорошее\nможет произойти с той, кому твоя поэма\nпонравится больше всего!", ok_action=Return())
    python:
        ...
        while True:
            ...
            for j in range(2):
                ...
                for i in range(5):
                    ...
                    ui.textbutton(word.word.replace("_", " "), clicked=ui.returns(word), text_style="poemgame_text", xpos=x, ypos=i * 56 + ystart)
                ui.close()
